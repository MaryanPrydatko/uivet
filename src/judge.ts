...
function buildPrompt(scenario: Scenario, rubric: string[]): string {
  ...
  const data = scenario.data === undefined ? "(none)" : JSON.stringify(scenario.data)
  return [
...
function parseJudge(raw: string, rubric: string[]): JudgeResult | null {
  ...
  let obj: { scores?: Record<string, unknown>; overall?: unknown; rationale?: unknown }
  try {
...
export async function judgeRun(
  ...
  const parsed = parseJudge(first, rubric) ?? parseJudge(await generateContent(model, req), rubric)
  if (parsed) return parsed
  const zero: Record<string, number> = {}
  for (const item of rubric) zero[rubricKey(item)] = 1
  return { scores: zero, overall: 1, rationale: "Judge returned unparseable JSON." }
}
