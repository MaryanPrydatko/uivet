...
const FENCE_BLOCK = /```(?:html)?\s*\n?([\s\S]*?)\n?```/i;
const FENCE_OPEN = /^```(?:html)?\s*/i;
const FENCE_CLOSE = /```\s*$/i;

export function stripFences(raw: string): string {
  const s = raw.trim();
  const fenced = s.match(FENCE_BLOCK);
  if (fenced?.[1]) {
    return fenced[1].trim();
  }
  return s.replace(FENCE_OPEN, "").replace(FENCE_CLOSE, "").trim();
}
  ...
  if (gen.kind === "module") {
    ...
    return async (scenario) => {
      ...
      if (typeof mod.generate !== "function") {
        throw new Error(
          `Generator module ${gen.path} must export generate(scenario)`
        );
      }
